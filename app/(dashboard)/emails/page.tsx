...
export default function EmailsPage() {
    ...
    const loadData = useCallback(async () => {
        ...
        const [sentRes, draftRes, tplRes] = await Promise.all([
            getEmails({ status: "sent",  ownerId: user.uid }),
            getEmails({ status: "draft", ownerId: user.uid }),
            getTemplates(),
        ]);
    ...
    const openSendNow = (email: Email) => {
        // Open the composer pre-loaded; user clicks Send
        setEditingEmail(email);
        setComposeOpen(true);
    };
    ...
    const handleUseTemplate = (template: EmailTemplate) => {
        setEditingEmail(undefined);
        // Open compose with subject+body pre-filled via context
        setComposeOpen(true);
        // We'll pass the template via context.subject / context.body
        // by setting a special initialEmail with just those fields pre-filled
        setEditingEmail({
            id: "",
            from: user?.email || "",
