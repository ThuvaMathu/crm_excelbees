...
function DealsPageContent() {
    ...
    const canEditDeals = canEditAll("deals");

    const canEditDeal = (deal: Deal): boolean => {
        if (canEditDeals) return true;
        return deal.ownerId === currentMember?.userId && can("deals", "edit");
    };
    ...
    const fetchDeals = async () => {
        try {
            setLoading(true);
            const { dealsByStage: deals, error } = await getDealsByStage(orgId);

            if (error) {
                logger.error("Error fetching deals", { module: "deals", action: "fetch", orgId, error });
                setDealsByStage({
                    Pipeline: [],
                    "Follow Up": [],
                    "Schedule Service": [],
            ...
            logger.error("Failed to fetch deals", { module: "deals", action: "fetch", orgId, error });
            setDealsByStage({
                Pipeline: [],
                "Follow Up": [],
                "Schedule Service": [],
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        const { success, error } = await updateDealStage(dealId, newStage, currentMember!.userId);
        if (success) {
                    ...
                    {STAGES.map((stage) => {
                        ...
                        return (
                            <KanbanColumn
                                ...
                                totalValue={canEditDeals ? `$${totalValue.toLocaleString()}` : "$•••"}
                                canMove={canEditDeal}
                                onDealClick={(dealId) => router.push(`${base}/deals/${dealId}`)}
                            />
                        );
                    })}
                </div>
